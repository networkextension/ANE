...
static NSString *gen_classifier_fwd(void) {
    ...
    [m appendFormat:@"        tensor<fp16, [%d,%d,1,1]> We = const()[name = tensor<string, []>(\"We\"), "
        "val = tensor<fp16, [%d,%d,1,1]>(BLOBFILE(path = tensor<string, []>(\"@model_path/weights/embed.bin\"), offset = tensor<uint64, []>(64)))];\n",
        VOCAB, DIM, VOCAB, DIM];
    [m appendFormat:@"        tensor<fp16, [1,%d,1,%d]> out = conv(dilations=dl,groups=gr,pad=pd,pad_type=pt,strides=st,weight=We,x=x)[name = tensor<string, []>(\"cls\")];\n", VOCAB, SEQ];
    [m appendString:@"    } -> (out);\n}\n"];
    return m;
}
...
static NSString *gen_classifier_bwd(void) {
    ...
    [m appendFormat:@"        tensor<int32, [3]> sh3 = const()[name = tensor<string, []>(\"sh3\"), val = tensor<int32, [3]>([1,%d,%d])];\n", VOCAB, SEQ];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d]> dl3 = reshape(shape=sh3,x=dl)[name = tensor<string, []>(\"rdl\")];\n", VOCAB, SEQ];
    // embed_t as baked constant [1, DIM, VOCAB]
    [m appendFormat:@"        tensor<fp16, [1,%d,%d]> Wet = const()[name = tensor<string, []>(\"Wet\"), "
        "val = tensor<fp16, [1,%d,%d]>(BLOBFILE(path = tensor<string, []>(\"@model_path/weights/embed_t.bin\"), offset = tensor<uint64, []>(64)))];\n",
        DIM, VOCAB, DIM, VOCAB];
    // matmul: [1, DIM, VOCAB] @ [1, VOCAB, SEQ] -> [1, DIM, SEQ]
    [m appendString:@"        tensor<bool, []> bF = const()[name = tensor<string, []>(\"bF\"), val = tensor<bool, []>(false)];\n"];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d]> dx3 = matmul(transpose_x=bF,transpose_y=bF,x=Wet,y=dl3)[name = tensor<string, []>(\"mm\")];\n", DIM, SEQ];
    // Reshape back to [1, DIM, 1, SEQ]
    [m appendFormat:@"        tensor<int32, [4]> sh4 = const()[name = tensor<string, []>(\"sh4\"), val = tensor<int32, [4]>([1,%d,1,%d])];\n", DIM, SEQ];
    [m appendFormat:@"        tensor<fp16, [1,%d,1,%d]> out = reshape(shape=sh4,x=dx3)[name = tensor<string, []>(\"out\")];\n", DIM, SEQ];
    [m appendString:@"    } -> (out);\n}\n"];
    return m;
}
...
static NSString *gen_softmax_vocab(void) {
    ...
    [m appendString:MIL_HDR];
    [m appendFormat:@"    func main<ios16>(tensor<fp16, [1, %d, 1, %d]> x) {\n", VOCAB, SEQ];
    [m appendString:@"        tensor<int32, []> ax = const()[name = tensor<string, []>(\"ax\"), val = tensor<int32, []>(1)];\n"];
    [m appendFormat:@"        tensor<fp16, [1,%d,1,%d]> out = softmax(axis=ax,x=x)[name = tensor<string, []>(\"sm\")];\n", VOCAB, SEQ];
    [m appendString:@"    } -> (out);\n}\n"];
    return m;
}
...
static NSString *gen_final_rmsnorm(void) {
    ...
    [m appendFormat:@"        tensor<fp16, [1,%d,1,1]> rw = const()[name = tensor<string, []>(\"rw\"), val = tensor<fp16, [1,%d,1,1]>(BLOBFILE(path = tensor<string, []>(\"@model_path/weights/rms_w.bin\"), offset = tensor<uint64, []>(64)))];\n", DIM, DIM];
    [m appendFormat:@"        tensor<fp16, [1,%d,1,%d]> out = mul(x=xr,y=rw)[name = tensor<string, []>(\"out\")];\n", DIM, SEQ];
    [m appendString:@"    } -> (out);\n}\n"];
    return m;
}
